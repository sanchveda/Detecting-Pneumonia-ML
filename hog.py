...
def processing_img(s,name, label):
    ...
    hog = Hog_descriptor(img, cell_size=16, bin_size=16)
    vector, image = hog.extract()
    img_hog = image.flatten()
    
    datas = np.array(img_hog)
...
np.save("hog_train_data.npy",train_data)
np.save("hog_train_label.npy",train_label)
test_data, test_label = fun(test)
np.save("hog_test_data.npy",test_data)
np.save("hog_test_label.npy",test_label)
